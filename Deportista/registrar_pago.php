...
// Obtener el ID del representante asociado al deportista
$stmt = $conn->prepare("SELECT ID_REPRESENTANTE FROM tab_representantes_deportistas WHERE ID_DEPORTISTA = :id_deportista");
$stmt->execute([':id_deportista' => $id_deportista]);
$resultado = $stmt->fetch(PDO::FETCH_ASSOC);
$id_representante = $resultado['ID_REPRESENTANTE'] ?? null;

if (!$id_representante) {
    $response = [
        'success' => false,
        'message' => 'No se encontró un representante asociado al deportista'
    ];
    ...
    try {
        ...
        $sql_pagos = "INSERT INTO tab_pagos (ID_REPRESENTANTE, ID_DEPORTISTA, ID_BANCO, METODO_PAGO, MONTO, FECHA_PAGO, MOTIVO, NOMBRE_ARCHIVO, ENTIDAD_ORIGEN,REGISTRADO_POR) 
                      VALUES (:id_representante, :id_deportista, :id_banco, :metodo_pago, :monto, :fecha_pago, :motivo, :nombre_archivo, :entidad_origen,'DEPO')";
