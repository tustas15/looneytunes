<?php
    ...
    $check_categoria=$check_categoria->query("SELECT id_categoria FROM tab_producto_categoria WHERE id_categoria='$category_id_del'");
    
    if($check_categoria->rowCount()==1){

    	$check_productos=conexion();
    	$check_productos=$check_productos->query("SELECT id_categoria FROM tab_productos WHERE id_categoria='$category_id_del' LIMIT 1");

    	if($check_productos->rowCount()<=0){

    		$eliminar_categoria=conexion();
	    	$eliminar_categoria=$eliminar_categoria->prepare("DELETE FROM tab_producto_categoria WHERE id_categoria=:id");
