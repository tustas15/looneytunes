...
try {
    ...
    $sql = "
        SELECT p.ID_PAGO, r.nombre_repre,r.apellido_repre, d.nombre_depo,d.apellido_depo, p.FECHA, p.TIPO_PAGO, p.MONTO, p.MOTIVO, p.BANCO
        FROM tab_pagos p
    ...
    if ($stmt->rowCount() > 0) {
        ...
        foreach ($pagos as $pago) {
            ...
            //echo '<td>' . htmlspecialchars($pago['nombre_repre']) . '</td>';
            ...
            echo '<td>' . ($pago['BANCO'] ? htmlspecialchars($pago['BANCO']) : 'N/A') . '</td>';
            echo '<td>
